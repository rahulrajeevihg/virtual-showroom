  ...
  return (
    <>
      ...
      <button
        ...
        className="absolute bottom-4 right-4 z-10 px-4 py-2 bg-white/20 hover:bg-white/40 text-white rounded-lg border border-white/30 shadow transition"
      >
        Quick View
      </button>
    </div>
    ...
    {showModal && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
        <div className="bg-black border-2 border-white/20 rounded-2xl p-8 max-w-md w-full relative">
          <button
            onClick={() => setShowModal(false)}
            className="absolute top-4 right-4 text-white hover:text-white/70 text-xl"
            aria-label="Close"
          >
            &times;
          </button>
          <div className="flex flex-col items-center">
            <div className="bg-white/10 rounded-xl h-48 w-48 mb-4 flex items-center justify-center">
              <svg className="w-16 h-16 text-white/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
            </div>
            <div className="text-xs text-white/50 uppercase tracking-wider mb-2">{product.category}</div>
            <h3 className="text-2xl font-bold text-white mb-2">{product.name}</h3>
            <p className="text-sm text-white/60 mb-4 text-center">{product.description}</p>
            ...
            <div className="flex items-center gap-2">
              ...
              {!product.inStock && (
                <span className="px-3 py-1 bg-white text-black text-xs font-bold rounded-full">Out of Stock</span>
              )}
            </div>
          </div>
        </div>
